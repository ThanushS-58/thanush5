            ...
            <CardContent className="p-0">
              <div className="space-y-4">
                ...
                <div className="space-y-4 text-left">
                  {/* Description */}
                  {(identificationResult.plant.translatedDescription || identificationResult.plant.description) && (
                    <div>
                  ...
                  {/* Medicinal Uses */}
                  {(identificationResult.plant.translatedUses || identificationResult.plant.uses) && (
                    <div>
                      <h5 className="font-semibold text-foreground mb-2">Traditional Uses:</h5>
                      <p className="text-muted-foreground">
                        {identificationResult.plant.translatedUses || identificationResult.plant.uses}
                      </p>
                    </div>
                  ...
                  {/* Preparation */}
                  {(identificationResult.plant.translatedPreparation || identificationResult.plant.preparation) && (
                    <div>
                      <h5 className="font-semibold text-foreground mb-2">Preparation:</h5>
                      <p className="text-muted-foreground">
                        {identificationResult.plant.translatedPreparation || identificationResult.plant.preparation}
                      </p>
                    </div>
                  ...
                  {/* Parts Used */}
                  {identificationResult.plant.partsUsed && (
                    <div>
                      <h5 className="font-semibold text-foreground mb-2">Parts Used:</h5>
                      <p className="text-muted-foreground">{identificationResult.plant.partsUsed}</p>
                    </div>
                  )}

                  {/* Properties */}
                  {identificationResult.plant.properties && (
                    <div>
                      <h5 className="font-semibold text-foreground mb-2">Properties:</h5>
                      <p className="text-muted-foreground">{identificationResult.plant.properties}</p>
                    </div>
                  )}

                  {/* Dosage */}
                  {identificationResult.plant.dosage && (
                    <div>
                      <h5 className="font-semibold text-foreground mb-2">Dosage:</h5>
                      <p className="text-muted-foreground">{identificationResult.plant.dosage}</p>
                    </div>
                  )}
                  ...
                  {(identificationResult.plant.translatedPrecautions || identificationResult.plant.precautions) && (
                    <div className="border-l-4 border-yellow-400 pl-4 bg-yellow-50 dark:bg-yellow-900/20 p-3 rounded">
                      <h5 className="font-semibold text-yellow-800 dark:text-yellow-200 mb-2">⚠️ Precautions:</h5>
                  ...
                  <div className="flex items-end">
                    <AudioButton 
                      text={`
                        ${identificationResult.plant.translatedName || identificationResult.plant.name}. 
                        ${identificationResult.plant.translatedDescription || identificationResult.plant.description}. 
                        Traditional uses include: ${identificationResult.plant.translatedUses || identificationResult.plant.uses}. 
                        ${identificationResult.plant.translatedPreparation ? `Preparation: ${identificationResult.plant.translatedPreparation}.` : identificationResult.plant.preparation ? `Preparation: ${identificationResult.plant.preparation}.` : ''}
                        ${identificationResult.plant.translatedPrecautions ? `Precautions: ${identificationResult.plant.translatedPrecautions}.` : identificationResult.plant.precautions ? `Precautions: ${identificationResult.plant.precautions}.` : ''}
                      `.trim()}
                      selectedVoice={selectedVoice}
