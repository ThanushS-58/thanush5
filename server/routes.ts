...
import { z } from "zod";
import { classifyPlantImage, searchPlantKnowledge, generateSpeech, identifyPlantWithDatabase, translatePlantInfo } from "./ai-services";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/plants/identify-enhanced", upload.single('image'), async (req, res) => {
    try {
      ...
      // Get enhanced identification with database matching
      const result = await identifyPlantWithDatabase(imageBase64, plantDatabase);
  ...
  app.post("/api/care-reminder", async (req, res) => {
    try {
      ...
      await storage.createNotification({
        ...
        type: 'care_reminder',
        title: `${careType} reminder for ${plantName}`,
